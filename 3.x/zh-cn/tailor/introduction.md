...
Tailor 是一项功能，用于定义网站使用的基于文件的内容结构，例如公司博客或团队页面。Tailor 自动生成用于管理记录的后端用户界面，并提供用于在前端显示和链接记录的 CMS 组件。

使用裁缝时，您可以跳过传统的插件开发工作流程，直接定义内容。字段仅定义为蓝图模板，内容存储在特殊的数据库表中。蓝图模板还可以指定导航和其他修饰符。
